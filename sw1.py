...
class ActionStates(StatesGroup):
    waiting_for_login_otp = State("ActionStates.waiting_for_login_otp")
    waiting_for_retry = State("ActionStates.waiting_for_retry")
...
def get_cheer_headers(tid, sid, token, user_id, device_id):
    return {
        "Host": "api.swiggy.com",
...
def get_vote_headers(device_id, tid, sid, token):
    return {
        "Host": "profile.swiggy.com",
...
def find_food_earned(data):
    if isinstance(data, dict):
        if 'totalEarnedByBL' in data and 'FOOD' in data['totalEarnedByBL']:
            return data['totalEarnedByBL']['FOOD']
        for key, value in data.items():
            res = find_food_earned(value)
            if res is not None:
                return res
...
async def send_log(message: str):
    try:
        await app.send_message(LOG_CHANNEL_ID, message, disable_web_page_preview=True)
...
async def is_user_registered(phone):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(url, headers=headers, json=payload) as resp:
                ...
                return data.get("data", {}).get("registered") is True
        except:
            return False
...
async def cmd_start(client: Client, message: Message):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🇮🇳 Cheer India (Free ₹75)", callback_data="mode_cheer")],
        [InlineKeyboardButton(text="🗳 Vote and Earn (₹100)", callback_data="mode_vote")],
        [InlineKeyboardButton(text="🚀 Combo: Cheer + Vote", callback_data="mode_both")]
    ])
...
async def process_phone(client: Client, message: Message):
    ...
    elif mode in ["cheer", "vote", "both"]:
        status_msg = await message.reply_text("🔄 Checking registration status...")
        is_reg = await is_user_registered(phone)
        if not is_reg:
            await status_msg.edit_text(
                "❌  <b>Not Registered!</b>\n\n"
                "This number is not on Swiggy.\n"
                "Please go back and use <b>Create Account</b> first.",
                parse_mode=enums.ParseMode.HTML
            )
            await state.clear()
            return
        await status_msg.delete()
        await send_otp_generic(message, state, phone, device_id, ActionStates.waiting_for_login_otp)
...
async def process_create_otp_verify(client: Client, message: Message):
    ...
    user_name_tg = message.from_user.first_name or "User"
            ...
            async with session.post(verify_url, headers=headers, params={"otp_source": "Sms-automatic"}, json=payload) as resp:
                ...
                async with session.post(signup_url, headers=signup_headers, json=signup_payload) as signup_resp:
                    ...
                    if signup_data.get("statusCode") == 0:
                        ...
                        log_msg = (
                            f"🆕 <b>[CREATE] Success</b>\n"
                            f"👤 <b>TG User:</b> <a href='tg://user?id={user_id_tg}'>{user_name_tg}</a>\n"
                            f"📱 <b>Mobile:</b> <code>{data['phone']}</code>"
                        )
        ...
        try:
            async with session.post(delete_url, headers=headers, json={"otp": otp}) as resp:
                ...
                if res.get("statusCode") == 33:
                    phone_no = data.get('phone', 'Unknown')
                    await message.reply_text(f"✅  <b>Account Deleted Successfully!</b>\nPhone: {phone_no}", parse_mode=enums.ParseMode.HTML)
                    
                    log_msg = (
                        f"🗑 <b>[DELETE] Success</b>\n"
                        f"👤 <b>TG User:</b> <a href='tg://user?id={user_id_tg}'>{user_name_tg}</a>\n"
                        f"📱 <b>Mobile:</b> <code>{phone_no}</code>"
                    )
                    await send_log(log_msg)
        ...
        try:
            ...
            async with session.post(submit_url, headers=camp_headers, json=submit_payload) as submit_resp:
                ...
                log_text = (
                    ...
                    f"👤 <b>TG User:</b> <a href='tg://user?id={user_id_tg}'>{user_name_tg}</a>\n"
                    f"📱 <b>Mobile:</b> <code>{data.get('phone')}</code>\n"
                    f"🎟 <b>Code Used:</b> <code>{ref_code}</code>\n"
                    f"📝 <b>Response:</b> {json.dumps(submit_res)}"
                )
...
# --- 5. ACTION (CHEER INDIA / VOTE) FLOW ---
@app.on_message(filters.text & state_filter(ActionStates.waiting_for_login_otp))
async def process_action_login_verify(client: Client, message: Message):
    state = FSMContext(message.from_user.id)
    otp = message.text.strip()
    data = await state.get_data()
    device_id = data['device_id']
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(verify_url, headers=headers, params={"otp_source": "Sms-automatic"}, json=payload) as resp:
                ...
                await state.update_data(
                    tid=tid, sid=sid, token=token,
                    customer_id=user_id, device_id=device_id
                )
                
                tg_user_id = message.from_user.id
                tg_user_name = message.from_user.first_name or "User"
                
                if mode == "cheer":
                    await redeem_cheer_benefit(status_msg, state, session, tg_user_id, tg_user_name)
                elif mode == "vote":
                    await execute_vote_and_earn(status_msg, state, session, tg_user_id, tg_user_name)
                elif mode == "both":
                    await redeem_cheer_benefit(status_msg, state, session, tg_user_id, tg_user_name)
...
async def redeem_cheer_benefit(message_obj, state, session, tg_user_id, tg_user_name):
    data = await state.get_data()
    redeem_url = "https://api.swiggy.com/api/v1/benefits/XvvE-jCYmU4/redeem?pollingCount=0"
    redeem_headers = get_cheer_headers(
        data['tid'], data['sid'], data['token'], data['customer_id'], data['device_id']
    )
    
    await message_obj.edit_text("🇮🇳 <b>Redeeming Cheer Offer (₹75)...</b>", parse_mode=enums.ParseMode.HTML)
    
    try:
        async with session.post(redeem_url, headers=redeem_headers) as redeem_resp:
            redeem_data = await redeem_resp.json()
            status_code = redeem_data.get("statusCode")
            
            if status_code == 0:
                attr = redeem_data.get("data", {}).get("benefitAttributes", {})
                const_fields = attr.get("constantFields", {})
                user_name = attr.get("username", "Unknown")
                success_text = const_fields.get("successText", "Offer Applied")
                info_text = const_fields.get("infoText", "Check your account")
                
                display_text = (
                    f"✅  <b>Benefit Redeemed Successfully!</b>\n\n"
                    f"👤 <b>Name:</b> {user_name}\n"
                    f"🎉 <b>Status:</b> REDEEMED\n"
                    f"ℹ️ <b>Message:</b> {success_text}\n"
                    f"📝 <b>Note:</b> {info_text}"
                )
                await message_obj.edit_text(display_text, parse_mode=enums.ParseMode.HTML)
                
                log_text = (
                    f"🇮🇳 <b>[CHEER 75] Success</b>\n"
                    f"👤 <b>TG User:</b> <a href='tg://user?id={tg_user_id}'>{tg_user_name}</a>\n"
                    f"📱 <b>Mobile:</b> <code>{data.get('phone')}</code>\n"
                    f"👤 <b>Name:</b> {user_name}\n"
                    f"✅  <b>Status:</b> REDEEMED"
                )
                ...
                if data.get("mode") == "both":
                    new_msg = await message_obj.reply_text("✅ Cheer completed. Preparing Vote & Earn...")
                    await asyncio.sleep(2)
                    await execute_vote_and_earn(new_msg, state, session, tg_user_id, tg_user_name)
                else:
                    await state.clear()
            else:
                error_msg = redeem_data.get("statusMessage", "Unknown Error")
                kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🔄 Try Again", callback_data="cheer_retry")],
                    [InlineKeyboardButton(text="❌  Exit", callback_data="action_exit")]
                ])
                await message_obj.edit_text(
                    f"❌  <b>Redemption Failed</b>\n\n"
                    f"⚠️ <b>Reason:</b> {error_msg}\n\n"
                    f"<i>Click 'Try Again' to retry without OTP.</i>",
                    parse_mode=enums.ParseMode.HTML,
                    reply_markup=kb
                )
                await state.set_state(ActionStates.waiting_for_retry)
    except Exception as e:
        await message_obj.edit_text(f"❌  Network Error: {str(e)}")
        await state.clear()

async def execute_vote_and_earn(message_obj, state, session, tg_user_id, tg_user_name):
    data = await state.get_data()
    url = "https://profile.swiggy.com/api/rx-awards/rx"
    params = {
        "category": "Andhra Food", "bu": "Food_Delivery",
        "cityId": "1", "getRewards": "true", "campaignId": "RXAWARD2026"
    }
    headers = get_vote_headers(data['device_id'], data['tid'], data['sid'], data['token'])
    ...
    while True:
        ...
        try:
            async with session.get(url, headers=headers, params=params) as resp:
                if resp.status != 200:
                    await message_obj.edit_text(f"❌ API Error: {resp.status}")
                    await state.clear()
                    break
                ...
                if food_rupees is None:
                    await message_obj.edit_text("❌ Could not parse food rupees from response.")
                    await state.clear()
                    break
                
                try:
                    fr_int = int(food_rupees)
                except ValueError:
                    fr_int = 0
                
                try:
                    await message_obj.edit_text(
                        f"🔄 <b>Voting... (Attempt {attempt})</b>\n💰 Earned so far: ₹{food_rupees}", 
                        parse_mode=enums.ParseMode.HTML
                    )
                except Exception:
                    pass
                ...
                if fr_int >= 100:
                    await message_obj.edit_text(
                        f"✅  <b>Target Reached!</b>\n🎉 You earned ₹{food_rupees}!", 
                        parse_mode=enums.ParseMode.HTML
                    )
                    log_text = (
                        f"🗳 <b>[VOTE & EARN] Success</b>\n"
                        f"👤 <b>TG User:</b> <a href='tg://user?id={tg_user_id}'>{tg_user_name}</a>\n"
                        f"📱 <b>Mobile:</b> <code>{data.get('phone')}</code>\n"
                        f"💰 <b>Earned:</b> ₹{food_rupees}"
                    )
                    await send_log(log_text)
                    await state.clear()
                    break
                
                if fr_int == last_amount:
                    stuck_count += 1
                else:
                    stuck_count = 0
                    last_amount = fr_int
                ...
                if stuck_count >= 7:
                    kb = InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔄 Retry Vote", callback_data="vote_retry")],
                        [InlineKeyboardButton("❌ Abort", callback_data="action_exit")]
                    ])
                    await message_obj.edit_text(
                        f"⚠️ <b>Offer might be having problems.</b>\n"
                        f"Amount stuck at ₹{food_rupees} for 7 attempts.\n"
                        f"Do you want to retry or abort?",
                        reply_markup=kb,
                        parse_mode=enums.ParseMode.HTML
                    )
                    await state.set_state(ActionStates.waiting_for_retry)
...
@app.on_callback_query(filters.regex("^cheer_retry") & state_filter(ActionStates.waiting_for_retry))
async def process_cheer_retry(client: Client, callback: CallbackQuery):
    state = FSMContext(callback.from_user.id)
    await callback.answer()
    tg_user_id = callback.from_user.id
    tg_user_name = callback.from_user.first_name or "User"
    async with aiohttp.ClientSession() as session:
        await redeem_cheer_benefit(callback.message, state, session, tg_user_id, tg_user_name)

@app.on_callback_query(filters.regex("^vote_retry") & state_filter(ActionStates.waiting_for_retry))
async def process_vote_retry(client: Client, callback: CallbackQuery):
    ...
    tg_user_name = callback.from_user.first_name or "User"
    async with aiohttp.ClientSession() as session:
        await execute_vote_and_earn(callback.message, state, session, tg_user_id, tg_user_name)

@app.on_callback_query(filters.regex("^action_exit") & state_filter(ActionStates.waiting_for_retry))
async def process_action_exit(client: Client, callback: CallbackQuery):
    state = FSMContext(callback.from_user.id)
    await state.clear()
