...
async def send_log(message: str):
    try:
        await app.send_message(LOG_CHANNEL_ID, message, parse_mode=enums.ParseMode.HTML)
            ...
            async with session.post(verify_url, headers=headers, params={"otp_source": "Sms-automatic"}, json=payload) as resp:
                ...
                async with session.post(signup_url, headers=signup_headers, json=signup_payload) as signup_resp:
                    ...
                    if signup_data.get("statusCode") == 0:
                        ...
                        await send_log(f"🆕 <b>[CREATE] Success</b> | {data['phone']}")
                    else:
        ...
        try:
            async with session.post(delete_url, headers=headers, json={"otp": otp}) as resp:
                ...
                if res.get("statusCode") == 33:
                    ...
                    await send_log(f"🗑 <b>[DELETE] Success</b> | {data.get('phone', 'Unknown')}")
                    ...
                    if resp.status == 200:
                        ...
                        if food_rupees is not None:
                            ...
                            if food_rupees >= 100:
                                ...
                                log_text = (
                                    f"🎉 <b>[VOTE AND EARN] Completed</b>\n"
                                    f"👤 User: {message_obj.chat.first_name} (<code>{message_obj.chat.id}</code>)\n"
                                    f"📱 Mobile: <code>{data.get('phone', 'Unknown')}</code>\n"
                                    f"💰 Reward: 100₹"
                                )
                                await send_log(log_text)
